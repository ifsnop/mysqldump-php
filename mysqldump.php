...
class SQLDump
{
	...
	/**
	 * Constructor of SQLDump. Note that in the case of an SQLite database connection, the filename must be in the $db parameter.
	 *
	...
	public function __construct($db = '', $user = '', $pass = '',
			$host = 'localhost', $type="mysql", $settings = null, $pdo_options = array(PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION))
	{
		$this->db = $db;
		$this->user = $user;
		$this->pass = $pass;
	...
	private function listValues($tablename)
	{
		...
		foreach ($this->dbHandler->query($stmt, PDO::FETCH_NUM) as $r) {
			...
			if ( ($lineSize > SQLDump::MAXLINESIZE) ||
